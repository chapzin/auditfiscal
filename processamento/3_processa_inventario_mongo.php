...
$db->entradas->remove();
$db->saidas->remove();
$db->inventario->remove();

$codigos = $db->command(["distinct" => "reg_C170", "key" => "COD_ITEM"]);
//var_dump($codigos);
$i = 0;
foreach ($codigos as $c170) {
    $entradas = $db->reg_C170->find(["IND_OPER" => 0, "COD_ITEM" => $c170[$i]]);

    foreach ($entradas as $entrada) {
        echo "Codigo:" . $entrada['COD_ITEM'] . " Custa:" . $entrada['VL_ITEM'] . "<br/>";
    }
}

//$entradas = $db->reg_C170->find(["IND_OPER"=>"0"]);
/*
